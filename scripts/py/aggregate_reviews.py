...
Usage: aggregate_reviews.py <project_dir>
"""
...
def usage():
    print("""Usage: aggregate_reviews.py <project_dir>

Aggregates review-*.json files into review-panel-summary.json.
Determines consensus, collects critical issues, builds prioritized actions.
...
def main():
    ...
    if not project_dir:
        ...
        print("Usage: aggregate_reviews.py <project_dir>", file=sys.stderr)
        sys.exit(1)
    ...
    for rf in review_files:
        ...
        if data:
            ...
            # Try to read and report error like bash version
            try:
                with open(rf) as f:
                    json.load(f)
