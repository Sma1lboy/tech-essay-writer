...
Usage: hook_workshop.py <project_dir> [style]
...
def usage():
    print("""Usage: hook_workshop.py <project_dir> [style]

Generates opening hook variations for articles and scores them on
engagement, relevance, and authenticity.

Styles: story, data, question, contrast, bold (default: all)

Reads: research-synthesis.json, materials.json, pipeline-state.json
Writes: hook-variations.json
...
def main():
    ...
    requested_style = sys.argv[2] if len(sys.argv) > 2 else "all"

    if not project_dir:
        print("ERROR: project_dir required", file=sys.stderr)
        print("Usage: hook_workshop.py <project_dir> [style]", file=sys.stderr)
        sys.exit(1)
