...
Usage: diagram_suggest.py <markdown_file> [--verbose]
"""
...
def usage():
    print("""Usage: diagram_suggest.py <markdown_file> [--verbose]
...
Output: JSON result to stdout (or verbose text with --verbose)""")
...
def main():
    ...
    if not os.path.isfile(md_file):
        print(json.dumps({"error": "file not found", "file": md_file}))
        sys.exit(1)

    # Determine state dir — look for .essay-state/ relative to the markdown file
    state_dir = ""
    parent = os.path.dirname(os.path.abspath(md_file))
    if os.path.basename(parent) == ".essay-state":
        state_dir = parent
    elif os.path.isdir(os.path.join(parent, ".essay-state")):
        state_dir = os.path.join(parent, ".essay-state")
    ...
    for section in sections:
        ...
        if sigs:
            suggestions.append({
                ...
                "type": "flowchart",
                "description": label(
                    "Flowchart showing the step-by-step process",
                    "流程图：展示分步过程"),
                "rationale": "; ".join(sigs),
        ...
        if sigs:
            suggestions.append({
                ...
                "description": label(
                    "Comparison table highlighting differences between approaches",
                    "对比表：突出不同方案之间的差异"),
                "rationale": "; ".join(sigs),
                "mermaid": None
            })
        ...
        if sigs:
            suggestions.append({
                ...
                "type": "er",
                "description": label(
                    "Entity-relationship diagram showing data model",
                    "ER图：展示数据模型关系"),
                "rationale": "; ".join(sigs),
        ...
        if sigs:
            suggestions.append({
                ...
                "type": "class",
                "description": label(
                    "Class diagram showing type hierarchy and relationships",
                    "类图：展示类型层级和关系"),
                "rationale": "; ".join(sigs),
        ...
        if sigs:
            suggestions.append({
                ...
                "type": "timeline",
                "description": label(
                    "Timeline showing progression or chronology",
                    "时间线：展示进展或时间顺序"),
                "rationale": "; ".join(sigs),
