...
# Usage: influence-score.sh <project_dir> <skill_dir> [verbose]
set -euo pipefail
...
# Read expertise graph JSON
EXPERTISE_JSON=$(bash "$SKILL_DIR/scripts/expertise-graph.sh" read 2>/dev/null || echo '{"topics":{}}')

python3 - "$STATE_DIR" "$VERBOSE" "$EXPERTISE_JSON" << 'PYEOF'
import json, sys, os, re

state_dir = sys.argv[1]
verbose = len(sys.argv) > 2 and sys.argv[2] == "verbose"
expertise_json = sys.argv[3] if len(sys.argv) > 3 else '{"topics":{}}'
...
if os.path.exists(research_file):
    try:
        ...
        # Novelty signals: unique angle, gap identification, freshness
        novelty = 5.0
...
tmp_file = os.path.join(state_dir, "influence-score.json.tmp." + str(os.getpid()))
