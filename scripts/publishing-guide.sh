...
# Usage: publishing-guide.sh <platform> [project_dir] [--format json]
set -euo pipefail

PLATFORM="${1:?platform required (internal|external|medium|devto|hashnode|wechat|juejin|all)}"
shift
...
if [ -n "$PROJECT_DIR" ] && [ -d "$PROJECT_DIR/.essay-state" ]; then
  STATE_DIR="$PROJECT_DIR/.essay-state"
fi

python3 - "$PLATFORM" "$FORMAT" "$STATE_DIR" << 'PYEOF'
import json, sys, os

platform = sys.argv[1]
fmt = sys.argv[2]
state_dir = sys.argv[3] if len(sys.argv) > 3 and sys.argv[3] else ""

# Read article metadata if state available
article_title = ""
article_tags = []
seo_keywords = []
article_description = ""
...
# Inject article metadata into guides
def enrich_guide(guide, title, tags, keywords, description):
    ...
    if tags:
        meta["suggested_tags"] = tags[:int(guide["limits"].get("tags_max", "5").split()[0]) if guide["limits"].get("tags_max", "5").split()[0].isdigit() else 5]
    if keywords:
