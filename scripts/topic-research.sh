...
# Usage: topic-research.sh <project_dir> <topic>
set -euo pipefail
...
def generate_research_questions():
    ...
    # 4. Conventional wisdom challenge
    questions.append({
...
def generate_search_queries():
    ...
    # Academic and deep-dive searches
    queries.append({
...
def identify_unique_angles():
    ...
    if len(detected_domains) > 1:
        ...
        # Pick an adjacent domain for cross-pollination
        adjacent_map = {
            "frontend": "backend",
            "backend": "frontend",
            "devops": "architecture",
...
for angle_entry in unique_angles:
    ...
    # Boost experience reports for practical topics
    if a_type == "experience_report" and any(d in detected_domains for d in ["devops", "backend", "data"]):
        score += 1.5
    # Boost contrarian for well-known topics (many keywords = well-known)
    if a_type == "contrarian" and len(keywords) <= 2:
        score += 1.0
    # Boost decision frameworks for architecture topics
    if a_type == "decision_framework" and "architecture" in detected_domains:
        score += 1.5
    # Boost bridge angle for topics with many tutorials
    if a_type == "bridge" and any(d in detected_domains for d in ["frontend", "mobile"]):
        score += 1.0
    # Boost domain-specific for specialized topics
    if a_type == "domain_practical":
        score += 1.0
    # Boost cross-domain if multiple domains detected
    if a_type == "cross_domain" and len(detected_domains) > 1:
        score += 1.5
    # Boost myth-busting for popular/mainstream topics
...
tmp_file = os.path.join(state_dir, "research-brief.json.tmp." + str(os.getpid()))
