...
# Usage: title-generator.sh <project_dir> [count]
set -euo pipefail
...
def formula_why_wrong():
    ...
    cw = conventional if conventional else f"everyone" if topic else "common advice"
...
tmp_file = os.path.join(state_dir, "title-variations.json.tmp." + str(os.getpid()))
